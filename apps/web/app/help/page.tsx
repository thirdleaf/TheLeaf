      ...
      <main>
           ...
           <div className="max-w-3xl mx-auto relative z-10 space-y-8">
              ...
              <div className="max-w-xl mx-auto relative">
                 <input 
                  ...
                  className="w-full bg-zinc-900 border border-white/5 rounded-2xl px-14 py-5 focus:outline-none focus:border-emerald-500/50 transition-all font-medium text-lg"
                 />
                 <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-zinc-500" size={24} />
                 {searchQuery && (
                   <button 
                    onClick={() => setSearchQuery("")}
                    className="absolute right-5 top-1/2 -translate-y-1/2 text-xs font-bold text-zinc-600 hover:text-white"
                   >
                     CLEAR
                   </button>
                 )}
        ...
        {!hasResults ? (
          <section className="py-20 text-center animate-in fade-in slide-in-from-bottom-4 duration-500">
             <div className="w-20 h-20 bg-zinc-900 border border-white/5 rounded-full flex items-center justify-center mx-auto mb-6 text-zinc-500">
                <Inbox size={40} />
             </div>
             <h3 className="text-xl font-bold mb-2">No results for &quot;{searchQuery}&quot;</h3>
             <p className="text-zinc-500 max-w-xs mx-auto">Try searching for broader terms like &quot;Broker&quot;, &quot;Security&quot;, or &quot;Tax.&quot;</p>
          </section>
        ) : (
          <>
            {/* Categories Grid */}
              ...
              <section className="py-20 px-6 max-w-7xl mx-auto">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {filteredCategories.map((cat) => (
                      <div key={cat.title} className="group p-8 bg-zinc-900/40 border border-white/5 rounded-[40px] hover:bg-zinc-900 transition-all hover:border-emerald-500/20 cursor-pointer h-full">
                ...
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="space-y-4">
                      {filteredFaqs.map((faq) => (
                        <div key={faq.q} className="p-8 bg-black/40 border border-white/5 rounded-3xl space-y-3">
                            <h4 className="font-bold text-white flex items-center gap-3">
                              <HelpCircle size={18} className="text-emerald-500" />
                              {faq.q}
                            </h4>
                            <p className="text-zinc-500 text-sm leading-relaxed ml-7">
                              {faq.a}
                            </p>
                        </div>
                      ))}
        ...
        <section className="py-32 px-6 text-center">
           <div className="max-w-2xl mx-auto space-y-10">
              ...
              <h2 className="text-3xl lg:text-5xl font-black text-white">Still need manual help?</h2>
              <p className="text-zinc-500 leading-relaxed">Our support pod is available Mon-Fri, 9AM - 6PM IST to help you tune your journal.</p>
